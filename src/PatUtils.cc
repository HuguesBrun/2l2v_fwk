...
namespace patUtils
{
  ...
  std::vector<pat::Muon> cleanMuonsFromDuplicates(pat::MuonCollection muons){
    unsigned int nsrc = muons.size();
    std::vector<int> good(nsrc, true);
    std::vector<pat::Muon> out;
   ...
   for (unsigned int i = 0; i < nsrc; ++i) {
       const pat::Muon &mu1 = muons.at(i);
       bool passthrough = mu1.isGlobalMuon() && mu1.numberOfMatches() >= 2;
       if (good[i] || passthrough) out.push_back(mu1);
   }

    return out;
  }

  bool isSameMuon(const pat::Muon &mu1, const pat::Muon &mu2)  {
      return (& mu1 == & mu2)  ||
  ...
  bool isBetterMuon(const pat::Muon &mu1, const pat::Muon &mu2) {
...
float computePFphotonIsolation(std::vector<pat::PackedCandidate> thePFparticles, float coneSize, float threshold, const pat::Muon &theMuon, pat::MuonCollection muons, reco::Vertex& vertex){
  ...
  for (unsigned int j = 0; j < muons.size(); ++j) {
      ...
      //cout << "hello found " <<listOfFSRphotonForThisMuon.size() << " photons" << endl;

  ...
  for(unsigned int f=0;f<urls.size();f++){
     ...
     for(ev.toBegin(); !ev.atEnd(); ++ev){ iev++;
	  ...
	  //	  if (!filterbadPFMuon || !filterbadChCandidate) continue;
          ...
          pat::MuonCollection muonsRAW; //before duplicate removal
          ...
          //if(PFparticles.isValid()){ puppimets = *puppimetsHandle;}
    			//GenJets
	 ...
	 if(isMC_ZZ2l2nu) mon.fillHisto("mzz", "qqZZ_atNNLO", mzz, weight);
         ...
         for(size_t ilep=0; ilep<leptons.size(); ilep++){
             ...
             if(is2016MC || is2016data){
                 ...
                 //isolation
                 passIso = lid==11?patUtils::passIso(leptons[ilep].el,  patUtils::llvvElecIso::Tight, patUtils::CutVersion::Spring15Cut25ns, 0) : patUtils::passIso(leptons[ilep].mu,  patUtils::llvvMuonIso::Tight, patUtils::CutVersion::Spring15Cut25ns,PFpartVect, muons, vertex);
                 passLooseLepton &= lid==11?patUtils::passIso(leptons[ilep].el,  patUtils::llvvElecIso::Loose, patUtils::CutVersion::Spring15Cut25ns, 0) : patUtils::passIso(leptons[ilep].mu,  patUtils::llvvMuonIso::Loose, patUtils::CutVersion::Spring15Cut25ns,PFpartVect, muons, vertex);

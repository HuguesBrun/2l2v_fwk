   ...
   //	      passIso &= (TMath::Max(gIso-gArea*rho,  0.0) < 0.5+0.005*pt); 
         ...
         for(size_t ivar=0; ivar<nvarsToInclude; ivar++){
           ...
           for(size_t ijet=0; ijet<jets.size(); ijet++){
             ...
             if(varyJesUp)     pt= jets[ijet].userFloat("jesup");
             if(varyJesDown)  pt= jets[ijet].userFloat("jesdown");
             if(varyJerUp)    pt= jets[ijet].userFloat("jerup");
             if(varyJerDown)  pt= jets[ijet].userFloat("jerdown");

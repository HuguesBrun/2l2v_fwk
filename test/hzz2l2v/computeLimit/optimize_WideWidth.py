...
for signalSuffix in signalSuffixVec : 
   ...
   #                     Gcut[c-1].SetPoint(mi, float(vals[0]), float(vals[c+1]));
	   ...
	   ##fvbf=0 for GGH and 1 for VBF
           ...
           if(ASYMTOTICLIMIT==True):
              ...
              SCRIPT.writelines("combine -M Asymptotic -m " +  str(m) + " workspace.root --setPhysicsModelParameters fvbf=1 --freezeNuisances fvbfi " + rLimit + " >>  COMB_VBF.log;\n") 
   ...
   ######################################################################
      ...
      print "root -l -b -q plotLimit.C+'(\""+DataCardsDir+"/Strenght_ggH_\",\""+DataCardsDir+"/LimitTree_ggH.root\",\"\", false, false, 13 , 35914.143 )'"
